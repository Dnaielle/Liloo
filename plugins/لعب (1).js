...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastwork < 600000) throw `⏱️ لا يمكنك العمل حتى ${msToTime(time - new Date())}`
...
function msToTime(duration) {
  ...
  return minutes + " ذقائق " + seconds + " ثواني" 
}
