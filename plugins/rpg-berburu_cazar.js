 ...
 *🐊 ${ar6} ${anti6}*		    *🐓 ${ar12} ${anti12}*`.trim();
  ...
  if (new Date - global.db.data.users[m.sender].lastberburu < 2700000) return conn.reply(m.chat, `يرجى اخذ الراحة لحظة لمواصلة الصيد\n\n⫹⫺ فترة الراحة ${clockString(time - new Date())}\n${wm}`, m);
...
[null, null]], null)*/
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['تحديد الهدف 🎯', 'الطعم 🍫 🍇 🍖', 'تم الكشف عن الحيوانات!! 🐂 🐅 🐘 🐼', 'تم الكشف عن الحيوانات!! 🐖 🐃 🐮 🐒'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 18000);
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['أسلحة جاهزة للصيد!!', 'اختبار الأسلحة🔫 💣 🪓 🏹', 'الرسوم البيانية للصيد!! 🚗 🏍️ 🚜', 'الطقس الجيد للصيد 🧤'].getRandom()}*`, null, {mentions: [m.sender]});
  }, 15000);
  ...
  setTimeout(() => {
    conn.reply(m.chat, `@${m.sender.split('@s.whatsapp.net')[0]} *${['البحث عن أدوات الصيد...', 'تجهيز كل شيء للصيد!!', 'تحديد موقع الصيد...', 'تجهيز مكان للصيد!!'].getRandom()}*`, m, m.mentionedJid ? {mentions: [m.sender]} : {});
  }, 0);
...
handler.tags = ['rpg'];
handler.command = /^(hunt|الصيد|caza(r)?)$/i;
handler.group = true;
